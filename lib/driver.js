...
var _respond = function(code, message){
  ...
  error.code = code || 101;
  error.message = "";
  if(config && config._errors) error.message += config._errors[code];
...
CasperDriver.prototype.run = function(req, cb){
  ...
  if(req instanceof Array){
    N = req.length;
    if(!N) return cb(null, 'Nothing to checkout');
    this.reqArray = req;
  } else {
    N = 1;
    this.reqArray = [req];
...
CasperDriver.prototype.spawn_child = function(i, cb){
   ...
   * spawn the child
   */
  this.children[i] = spawn('casperjs', ["--web-security=no", path.join(__dirname, '../index.js'), reqStr]);
