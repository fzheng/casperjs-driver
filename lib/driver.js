...
var io = require('socket.io').listen(29110, {log: true});
...
CasperDriver.prototype.run = function(req, cb){
  ...
  io.sockets.on('connection', function(socket){
    _me.socket = socket;
    socket.emit('from_node', {body : "Hello from node", reply: true});
    socket.on('from_casper', function(data){
      logger.info(">>> node driver.js received msg: " + JSON.stringify(data));
      if(data.reply) {
        socket.emit('from_node', {body: 'Nice to meet you', reply: false});
      } else {
...
CasperDriver.prototype.teardown = function(cb){
  ...
  return cb(null, "cleaned up processes. exit.");
};
